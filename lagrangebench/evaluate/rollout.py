...
def _eval_batched_rollout(
    ...
    params: hk.Params,
    state: hk.State,
    traj_batch_i: Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray],
    ...
    pos_input_batch, particle_type_batch, times_batch = traj_batch_i
    ...
    while step < n_rollout_steps + n_extrap_steps:
        ...
        rho_deviations = calculate_densities_batched(features_batch, particle_type_batch, kernel, radius, mass_particle, n_nodes_max)
...
def eval_rollout(
    ...
    for i, traj_batch_i in enumerate(loader_eval):
        ...
        if n_traj_left < batch_size:
            traj_batch_i = jax.tree_util.tree_map(lambda x: x[:n_traj_left], traj_batch_i)

        # numpy to jax
        traj_batch_i = jax.tree_util.tree_map(lambda x: jnp.array(x), traj_batch_i)
...
def infer(
    ...
    # init values
    pos_input_and_target, particle_type, time_frames = next(iter(loader_test))
    sample = (pos_input_and_target[0], particle_type[0], time_frames[0])
